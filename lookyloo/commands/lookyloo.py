...
from ..constants import HISTORY_FILENAME, ALL_CAMERAS, AUTO_EXPORT_CAMERAS, LOOKYLOO_DATA_ROOTS, QUICKLOOK_PATH, DEFAULT_CUBE, DEFAULT_SEPARATE, CHECK_INTERVAL_SEC, LOG_PATH
...
def main():
    ...
    parser.add_argument('-C', '--cube-mode-all', help="Whether to write all archives as cubes, one per XRIF, regardless of the default for the device (implies --omit-telemetry)", action='store_true')
    parser.add_argument('-S', '--separate-mode-all', help="Whether to write all archives as separate FITS files regardless of the default for the device", action='store_true')
    ...
    parser.add_argument('-c', '--camera', help=f"Camera name (i.e. rawimages subfolder name), repeat to specify multiple names. (default: {AUTO_EXPORT_CAMERAS})", action='append')
    ...
    log_file_path = f"./lookyloo_{timestamp_str}.log" if args.verbose or args.dry_run else None
